...
public class GrammyAnalyst {
	...
	void loadArtists() {
		...
		for (Map.Entry<String, List<Nomination>> n: grammyMap.entrySet()) {
			Artist tmpArt = new Artist(n.getValue().get(0).artist, n.getValue());
			artists.add(tmpArt);
		}
	}
	...
	void printGrammyReport() {
		...
		int sign = 1;
		for (Nomination n: nominations) {
			System.out.println(sign + ". " + n.artist + ": " + n.category + ": " + n.title);
			sign++;
		}
	}
	...
	void searchGrammys(String artist) {
		...
		int sign = 0;
		for (Map.Entry<String, List<Nomination>> n: grammyMap.entrySet()) {
			String[] tmpArtist = n.getKey().split(",");
			for (String s: tmpArtist) {
				if (artist.toLowerCase().equals(s.trim().toLowerCase())) {
					for (Nomination x: n.getValue()) {
						System.out.println(x.artist + ": " + x.category + ": " + x.title);
					}
					sign = 1;
					break;
				}
