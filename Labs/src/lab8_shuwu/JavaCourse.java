...
public class JavaCourse implements Runnable{
	...
	public void run() {
		...
		//java course created students every xx minutes
		while(!allDone){
			synchronized (studentQ){
				studentQ.offer(new Student());
			}
			try{
				maxQLength = Math.max(maxQLength, studentQ.size());
		...
		//鍋滀笅鏉ヤ箣鍚� 绛夊緟鎵�鏈塼a绾跨▼join
		for(int i = 0; i < taThreads.size(); i++){
			try {
				taThreads.get(i).join();
			}catch (InterruptedException e){
				e.printStackTrace();
			}
		}
